...
# ``<repo>/tools/*.so``, and an author-specific ``<workspace>/refs/...`` for llama.cpp). They now default under
...
def default_bin_dir() -> str:
    ...
    ``<workspace>/refs/PrismML-llama.cpp/build/bin`` if that is where it was built."""
