...
(`<workspace>/refs/PrismML-llama.cpp`) registers only `cohere2`, so `load_arch` throws
...
```sh
cd <workspace>/refs/PrismML-llama.cpp
...
BIN=<workspace>/refs/PrismML-llama.cpp/build/bin/llama-tokenize
